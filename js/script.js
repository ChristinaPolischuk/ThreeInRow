window.onload = function () {
  ...
  let color = [1, 2, 3, 4, 5]; // количество цветов фишек
  let timer;
  let score = 0;
  let flag; // проверка когда запускать следующий блок
  ...
  function draw() {
    ...
    tetrisField.innerHTML = out; // перерисовка игрового поля
  ...
  function checkLine() {
    for (let i = tetris.length - 1; i >= 0; i--) {
      for (let j = 0; j < tetris[i].length; j++) {
        if (
          tetris[i][j] > 10 &&
          tetris[i][j + 1] != undefined &&
          tetris[i][j + 2] != undefined
        ) {
          if (
            tetris[i][j] == tetris[i][j + 1] &&
            tetris[i][j] == tetris[i][j + 2]
          ) {
            tetris[i][j] = 0;
            tetris[i][j + 1] = 0;
            tetris[i][j + 2] = 0;
            ...
            // m - вышележащие строки
            for (let m = i; m >= 0; m--) {
              //проверяем если блок неподвижен
              if (tetris[m][j] > 10) {
                // делаем его движимым
  ...
  function finish() {
    ...
    for (let i = tetris.length - 1; i >= 0; i--) {
      for (let j = 0; j < tetris[i].length; j++) {
        stop = true;
        // k - перебор горизонтального ряда
  ...
  function createForm() {
    ...
    form.innerHTML = '<h3 class="title">Поделиться результатом</h3>';
    ...
    submit.setAttribute("value", "Отправить результат");
  ...
  document.onkeydown = function (e) {
    ...
    // }
    // return false;
    if (e.code == "ArrowRight") tetrisRight();
    if (e.code == "ArrowLeft") tetrisLeft();
  };
};